...
class AuthService:
    ...
    async def authenticate_user(
        ...
        stmt = (
            select(User).options(selectinload(User.user_roles).selectinload(UserRole.role)).where(User.login == login)
        )
        result = await self.db.execute(stmt)
    ...
    async def logout_all(
        ...
        logger.info(f"User logged out from all devices: {user_id} (new version: {new_version})")
    ...
    async def verify_token(
        ...
        if token_type == "access":
            ...
            if current_version is not None and int(current_version) != token_version:
                logger.warning(
                    f"Token version mismatch for user {user_id}: " f"token={token_version}, current={current_version}"
                )
                raise TokenInvalidError()
